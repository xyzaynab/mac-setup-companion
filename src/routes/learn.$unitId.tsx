    ...
    <Shell>
      ...
      <div className="mt-7 grid gap-8 xl:grid-cols-[minmax(0,1fr)_220px]">
        <div className="min-w-0">
          ...
          <h2 className="mt-2 text-[24px] font-semibold leading-snug tracking-tight">
            {current.title}
          </h2>
          {current.lead && (
          ...
          <div className="mt-7 max-w-[66ch]">
            ...
            <textarea
              ...
              className="mt-2 w-full resize-y border border-border bg-input px-3 py-2.5 text-[15px] leading-relaxed outline-none focus:border-ring"
            />
          </div>
          ...
          <div className="mt-6 flex max-w-[66ch] flex-wrap items-center gap-2 border-t border-border pt-4">
            <button
              disabled={index === 0}
              onClick={() => go(index - 1)}
              className="border border-border px-3.5 py-2 text-[15px] hover:bg-secondary disabled:opacity-40"
            >
              Back
            </button>
            <button
              onClick={() => progressActions.toggleDone(unit.id, current.id)}
              className={`border px-3.5 py-2 text-[15px] ${
                stepState === "done"
            ...
            <button
              ...
              className="border border-border px-3.5 py-2 text-[15px] text-muted-foreground hover:bg-secondary"
            >
              Skip for now
            </button>
            <button
              onClick={() => setHelp((h) => !h)}
              className="border border-border px-3.5 py-2 text-[15px] text-violet hover:bg-secondary"
            >
              I need help
            </button>
            {index < unit.steps.length - 1 ? (
              <button
                onClick={() => go(index + 1)}
                className="ml-auto bg-primary px-4 py-2 text-[15px] font-medium text-primary-foreground hover:opacity-90"
              >
                Next
              </button>
            ) : nextUnit ? (
        ...
        <nav className="hidden xl:block">
            ...
            {unit.steps.map((s, i) => {
              ...
              return (
                <button
                  key={s.id}
                  onClick={() => go(i)}
                  className={`flex w-full items-start gap-2 px-2 py-1.5 text-left text-[13px] leading-snug ${
                    i === index
          ...
          <button
            ...
            className="mt-4 px-2 text-[12px] text-muted-foreground hover:text-destructive"
          >
            Reset this section
          </button>
        </nav>
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center gap-2">
        ...
        {unit.phase && <span className="text-[12px] text-muted-foreground">{unit.phase}</span>}
      </div>
      <h1 className="mt-2 text-[17px] font-medium text-muted-foreground">{unitLabel(unit)}</h1>
      {unit.summary && (
        <p className="mt-1 max-w-[66ch] text-[15px] leading-relaxed text-muted-foreground">
          {unit.summary}
        </p>
      )}
    </div>
    ...
    <div className={inline ? "mt-3 space-y-1" : "mt-7 max-w-[66ch] space-y-1"}>
      ...
      {units.map((u) => (
        <Link
          ...
          className="block border border-border px-3.5 py-2.5 text-[15px] hover:bg-secondary"
        >
          {u.title}
          {u.summary && (
